...
float capped_torus(vec3 pt) {
    pt = pt - vec3(0, 0, -12.5);
    vec3 pt2 = pt - vec3(0, 0,6.2);
    float cube = cube(pt2, 2)-1;
    float torus = torus(pt);

    return max(torus, -cube);
}
...
float building2(vec3 pt) {
    pt.y = pt.y - clamp(round(pt.y), 0, 5);
    return capped_torus(pt);
}
...
float task6(vec3 pt) {
    ...
    float torus = building2(pt);

    return min(min(of, plane), min(torus, fa));
}
